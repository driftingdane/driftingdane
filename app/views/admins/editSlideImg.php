<div class="card card-body bg-light mb-5">
    <?php
    if (empty($data['slideById']->sl_img)) {$setImg = "nologo.png";
    } else {$setImg = $data['slideById']->sl_img;}
    ?>
    <?php echo flash('resume_message'); ?>
<form action="<?php echo URLROOT . '/admins/uploadImgSlide/' . $data['slideById']->sl_id; ?>" class="icon-form process"
    ...
    <?php if (!empty($data['slideById']->sl_img)) : ?>
        ...
        <div class="userAvatar mt-1">
            <img class="img-fluid mx-auto d-block" src="<?php echo URLROOT; ?>/sliderImg/<?php echo $setImg; ?>">
        </div>
